...
const {xorBuffer, compareDigest, splitAtom, splitCommaEqual, isPlainObject, loopKeys} = require(path.join(__dirname, '/helper.js'))
var Err = require(path.join(__dirname, '/error.js'))
var Cursor = require(path.join(__dirname, '/cursor.js'))
var ReadableStream = require(path.join(__dirname, '/stream.js'))
var Metadata = require(path.join(__dirname, '/metadata.js'))

var protodef = require(path.join(__dirname, '/protodef.js'))
...
var NULL_BUFFER = Buffer.from('\0', 'binary')
var CACHE_PBKDF2 = {}
...
function Connection (
  r,
  {
    ...
    timeoutConnect = r._timeoutConnect,
    // The connection will be pinged every <pingInterval> seconds
    pingInterval = r._pingInterval,
    db,
    connection,
    ssl = false,
    tlsOptions = {}
  ...
  const options = {user, password, authKey, host, port}

  if (authKey && password) throw new Err.ReqlDriverError('Cannot use both authKey and password')
...
//   if (options.db) this.db = options.db // Pass to each query
  ...
  const metadata = {}
  let open = false // true only if the user can write on the socket
  let timeout = null
  ...
  if (connection === undefined) {
    ...
    if (ssl) {
      tlsArgs = isPlainObject(tlsOptions) ? {host, port, family, ...tlsOptions} : connectOpts;
      socket = tls.connect(connectOpts)
    } else {
      socket = net.connect({host, port, family})
    }
  }
  socket.setKeepAlive(true)


  const timeoutOpen = setTimeout(function () {
    socket.end() // Send a FIN packet
    reject(new Err.ReqlDriverError('Failed to connect to ' + host + ':' + port + ' in less than ' + timeoutConnect + 's').setOperational())
  }, timeoutConnect * 1000)
  ...
  connection.on('connect', function () {
    ...
    var authBuffer = Buffer.from(JSON.stringify({
    ...
    try {
      socket.write(Buffer.concat([versionBuffer, authBuffer, NULL_BUFFER]))
    } catch (err) {
  ...
  socket.on('timeout', function (buffer) {
    socket.open = false
    self.emit('timeout')
  })
...
_performHandshake = function (handshake, reject) {
  const self = this
  ...
  switch (self.state) {
    ...
    case STATE_COMPUTE_SALTED_PASSWORD:
      self._computeSaltedPassword(handshake, reject)
      return
    case STATE_COMPARE_DIGEST:
      self._compareDigest(handshake)
  }
}
...
Connection.prototype._computeSaltedPassword = function (handshake, reject) {
  var self = this
  var authentication = helper.splitCommaEqual(handshake.authentication)

  var randomNonce = authentication.r
  var salt = Buffer.from(authentication.s, 'base64')
...
_send (query, token, resolve, reject, originalQuery, options, end) {
  if (open === false) {
    return reject(new Err.ReqlDriverError('The connection was closed by the other party').setOperational())
  }
...
_continue (token, resolve, reject) {
  const query = [protodef.Query.QueryType.CONTINUE]
