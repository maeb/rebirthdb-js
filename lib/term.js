...
Term.prototype.run = function (connection, options, callback) {
  ...
  if (helper.isPlainObject(connection) && (typeof connection._isConnection === 'function') && (connection._isConnection() === true)) {
    ...
    const poolMaster = self._r.getPoolMaster() // if self._r is defined, so is self._r.getPool()
    if (!poolMaster) {
...
Term.prototype.time = function () {
  ...
  if ((_args.length !== 4 && _args.length !== 7) || _args.some(isArg)) {
  ...
  const args = _args.map((arg) => new Term(this._r).expr(arg))
  term._fillArgs(args)
  return term
}
...
Term.prototype.line = function () {
  ...
  const args = Array.from(arguments.map((arg) => new Term(this._r).expr(arg)))
  term._fillArgs(args)
  return term
}
...
Term.prototype.wait = function (options) {
  var self = this
  if (self._fastArityRange(arguments.length, 0, 1) === false) {
    self._arityRange([...arguments], 0, 1, 'wait', self)
  }
  var term = new Term(self._r)
...
Term.prototype.reconfigure = function (config) {
  ...
  } else if (!helper.isPlainObject(config)) {
  ...
  helper.loopKeys(config, function (obj, key) {
    switch (key) {
      ...
      case 'replicas':
      case 'dryRun':
      case 'primaryReplicaTag':
      case 'nonvotingReplicaTags':
...
Term.prototype.grant = function (name, access) {
  if (this._fastArity(arguments.length, 2) === false) {
    this._arity([...arguments], 2, 'grant', this)
  }
...
function isArg (arg) {
  return (arg instanceof Term) && (arg._query[0] === termTypes.ARGS)
}

Term.prototype._arityRange = function (args, min, max, method, term) {
  const tooFew = args.length < min
  const tooMany = args.length > max

  if (tooFew || tooMany) {
    const superlative = tooFew ? 'least' : 'most'
    const plural = min > 1 ? 's' : ''

    if (args.some(isArg)) {
      throw new Error.ReqlDriverError('`' + method + '` takes at ' + superlative + ' ' + min + ' argument' + plural + ', ' + args.length + ' provided', term._query)
    }
  }
}
