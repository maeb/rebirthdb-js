...
function Connection (r, options, resolve, reject) {
    ...
    if (self.open === false) {
      for (let i = 0; i < self.buffer.length; i++) {
        if (self.buffer[i] === 0) {
          ...
          try {
            const handshake = JSON.parse(handshakeStr)
            self._performHandshake(handshake, resolve, reject)
          } catch (error) {
            self._abort()
            reject(new Err.ReqlDriverError('Could not parse the message sent by the server : \'' + handshakeStr + '\'').setOperational())
          }
        }
      }
    } else {
  ...
  self.connection.toJSON = function () {
...
Connection.prototype._performHandshake = function (handshake, resolve, reject) {
  const self = this

  if (handshake.success !== true) {
    self._abort()
    reject(new Err.ReqlDriverError('Error ' + handshake.error_code + ':' + handshake.error).setOperational())
    return
  }
  try {
    switch (self.state) {
      case 0:
        self._checkProtocolVersion(handshake, reject)
        break
      case 1:
        self._computeSaltedPassword(handshake, reject)
        break
      case 2:
        self._compareDigest(handshake, resolve, reject)
        self.connection.removeAllListeners('error')
        self.open = true
...
Connection.prototype._checkProtocolVersion = function (msg, reject) {
  // Expect max_protocol_version, min_protocol_version, server_version, success
  const minVersion = msg.min_protocol_version
  const maxVersion = msg.max_protocol_version
  if (minVersion > PROTOCOL_VERSION || maxVersion < PROTOCOL_VERSION) {
    console.log('throwing error because proto version')
...
Connection.prototype._computeSaltedPassword = function (handshake) {
  var self = this
  ...
  if (CACHE_PBKDF2.hasOwnProperty(cacheKey)) {
    ...
    crypto.pbkdf2(self.password, salt, iterations, KEY_LENGTH, 'sha256', function (error, saltedPassword) {
      if (error != null) {
        throw new Err.ReqlDriverError('Could not derive the key. Error:' + error.toString()).setOperational()
      }
